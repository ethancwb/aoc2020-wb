...
def day11a(input):
  lst = input.split('\n')
  matrix = [[spot for spot in line] for line in lst]
  n, m = len(matrix), len(matrix[0])
  nextBoard = [['.'] * m for i in range(n)]
  ...
  while changed:
    ...
    for i in range(n):
      for j in range(m):
        ...
        if matrix[i][j] == 'L' and adjacentCount(matrix, i, j) == 0:
          nextBoard[i][j] = '#'
          changed = True
        elif matrix[i][j] == '#' and adjacentCount(matrix, i, j) >= 4:
          nextBoard[i][j] = 'L'
          changed = True
        if nextBoard[i][j] == '#':
          count += 1
...
def adjacentCountB(matrix, i, j, d):
  if 0 <= i < len(matrix) and 0 <= j < len(matrix[0]):
    currPos = matrix[i][j]
    if currPos != '.':
      return 1 if currPos == '#' else 0
    return adjacentCountB(matrix, i + pos[d][0], j + pos[d][1], d)
  return 0
...
def day11b(input):
  ...
  while changed:
    ...
    for i in range(n):
      for j in range(m):
        nextBoard[i][j] = matrix[i][j]
        if matrix[i][j] == 'L' and sum(adjacentCountB(matrix, i + pos[d][0], j + pos[d][1], d) for d in range(8)) == 0:
          nextBoard[i][j] = '#'
          changed = True
        elif matrix[i][j] == '#' and sum(adjacentCountB(matrix, i + pos[d][0], j + pos[d][1], d) for d in range(8)) >= 5:
          nextBoard[i][j] = 'L'
          changed = True
        if nextBoard[i][j] == '#':
          count += 1
...
input = """LLLLL.LLLLLL.LLLLLLLLL.LLL.LL.LLLLLLL.LLLLLLLLLLLLLLLLLLLLLLLLLLL.LLLLLLLLLLL.LLLLLLLLLL.LLLLLLL
