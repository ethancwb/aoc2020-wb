def day6a(input):
  return sum(len({char for line in g.split('\n') for char in line}) for g in input.split('\n\n'))
...
def day6b(input):
  ...
  for g in input.split('\n\n'):
    ans = set()
    for idx, line in enumerate(g.split('\n')):
      charSet = set(list(line))
      if idx == 0:
        ans = charSet
      else:
        ans = ans & charSet
...
input = """rypdeiqkbgacnxwumhtozfjvs
