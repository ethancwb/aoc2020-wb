...
def day7a(input):
  ...
  for re in input.split('\n'):
    formatted = split(" bags| bag| (\d+) ", re)
    head = formatted[0]
    for i in range(4, len(formatted), 4):
      inDegree[formatted[i]].append(head)
  ans, stk = set(), ['shiny gold']
  while stk:
    curr = stk.pop()
    stk.extend(inDegree[curr])
    ans.update(set(inDegree[curr]))
  return len(ans)
...
def day7b(input):
  ...
  for re in input.split('\n'):
    ...
    for i in range(4, len(formatted), 4):
      ...
      outDegree[head].append((formatted[i], int(formatted[i-1])))
  count, stk = 0, [('shiny gold', 1)]
  while stk:
    curr, factor = stk.pop()
    stk.extend(list(map(lambda x: (x[0], x[1] * factor), outDegree[curr])))
    count += factor
  return count - 1
